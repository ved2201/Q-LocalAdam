...
class MidResNet18_CIFAR100(nn.Module):
    def __init__(self):
        super().__init__()
        self.inplanes = 64
...
def run_federated_experiment(
    ...
    global_model = MidResNet18_CIFAR100().to(device)
    ...
    for round_num in range(1, num_rounds + 1):
        ...
        for client_id in range(num_clients):
            local_model = MidResNet18_CIFAR100().to(device)
...
def plot_all_results(all_results):
    ...
    ax.set_title("IID vs Non-IID Comparison (CIFAR-100)", fontsize=16, fontweight='bold')
    ax.grid(True, alpha=0.3)
    ax.legend(loc='lower right')
    plt.tight_layout()
    fig1.savefig("figures/cifar100/1_iid_vs_noniid.png", dpi=300, bbox_inches="tight")
    fig1.savefig("figures/cifar100/1_iid_vs_noniid.pdf", bbox_inches="tight")
    plt.close(fig1)
    ...
    ax.set_xlabel("Federated Round")
    ax.set_ylabel("Test Accuracy (%)")
    ax.set_title("Main Comparison: Vanilla-ClientAdam vs Q-LocalAdam (α=0.1, CIFAR-100)", fontsize=16, fontweight='bold')
    ax.grid(True, alpha=0.3)
    ax.legend(loc='lower right')
    plt.tight_layout()
    fig2.savefig("figures/cifar100/2_main_comparison.png", dpi=300, bbox_inches="tight")
    fig2.savefig("figures/cifar100/2_main_comparison.pdf", bbox_inches="tight")
    plt.close(fig2)
    ...
    ax.set_xlabel("Federated Round")
    ax.set_ylabel("Test Accuracy (%)")
    ax.set_title("Necessity of Log-Space Quantization (CIFAR-100)", fontsize=16, fontweight='bold')
    ax.grid(True, alpha=0.3)
    ax.legend(loc='lower right')
    plt.tight_layout()
    fig3.savefig("figures/cifar100/3_naive_comparison.png", dpi=300, bbox_inches="tight")
...
CHECKPOINT_PATH = "results/cifar100/checkpoint.json"


def save_checkpoint(all_results, next_experiment_idx):
    os.makedirs("results/cifar100", exist_ok=True)
    payload = {
        "all_results": all_results,
        "next_experiment_idx": next_experiment_idx,
        "timestamp": time.time(),
    }
...
if __name__ == "__main__":
    ...
    experiments = [
        ...
        {
            "key": "vanilla_noniid",
            "desc": "Vanilla-ClientAdam (α=0.1, Extreme Non-IID)",
            "kwargs": dict(
        ...
        {
            "key": "naive_int8",
            "desc": "Naive INT8 (Linear Quantization)",
            "kwargs": dict(
        ...
        {
            "key": "momentum_only",
            "desc": "Component Ablation (Momentum Only)",
            "kwargs": dict(
        ...
        {
            "key": "variance_only",
            "desc": "Component Ablation (Variance Only)",
            "kwargs": dict(
        ...
        {
            "key": "noniid_alpha05",
            "desc": "Non-IID Robustness (α=0.5, Moderate)",
            "kwargs": dict(
                method_name="Q-LocalAdam",
                num_rounds=120,
                alpha=0.5,
        ...
        {
            "key": "noniid_alpha10",
            "desc": "Non-IID Robustness (α=1.0, Mild)",
            "kwargs": dict(
                method_name="Q-LocalAdam",
                num_rounds=120,
                alpha=1.0,
        ...
        {
            "key": "block_128",
            "desc": "Block Size Ablation (B=128)",
            "kwargs": dict(
                method_name="Q-LocalAdam",
                num_rounds=120,
                alpha=0.1,
        ...
        {
            "key": "lr_5e4",
            "desc": "Learning Rate Sensitivity (lr=5e-4)",
            "kwargs": dict(
                method_name="Q-LocalAdam",
                num_rounds=120,
                alpha=0.1,
    ...
    all_results, start_idx = load_checkpoint()
    ...
    for i in range(start_idx, num_experiments):
        ...
        if key in all_results:
            ...
            save_checkpoint(all_results, i + 1)

    print("\n" + "="*70)
    print("SAVING RESULTS AND GENERATING FIGURES (CIFAR-100)")
    print("="*70)
